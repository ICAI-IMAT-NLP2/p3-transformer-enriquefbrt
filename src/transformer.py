    ...
    def __beam_search_decode(
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                log_probs = logits.log_softmax(dim=-1)  # Shape: (1, vocab_size)
                for next_token in range(log_probs.size(1)):
                    new_seq = torch.cat(
                        [
                            seq,
                            torch.full(
                                (1, 1), next_token, dtype=torch.long, device=device
                            ),
                        ],
                        dim=1,
                    )  # Shape: (1, seq_len+1)
    ...
    def __contrastive_decode(
        ...
        **kwargs,
        ...
        for _ in range(max_length):
            ...
            # Prepare tensors for all candidates across the batch
            # topk_indices/topk_probs: (batch_size, k)
            # Expand tgt_input and enc_output so we have B*k candidates stacked on dim 0
            expanded_tgt_input = tgt_input.repeat_interleave(
                k, dim=0
            )  # Shape: (batch_size * k, seq_len)
            next_tokens = topk_indices.reshape(-1, 1)  # Shape: (batch_size * k, 1)
            y_candidates = torch.cat(
                [expanded_tgt_input, next_tokens], dim=1
            )  # Shape: (batch_size * k, seq_len + 1)

            # Repeat encoder outputs for each candidate
            enc_output_expanded = enc_output.repeat_interleave(
                k, dim=0
            )  # Shape: (batch_size * k, src_seq_len, hidden)

            # Pass each candidate through the decoder
            dec_outputs_candidate = self.decoder(y_candidates, enc_output_expanded)

            # Extract hidden states
            h_v = dec_outputs_candidate[
                :, -1, :
            ]  # Shape: (batch_size * k, hidden_size)
            h_j = dec_outputs_candidate[
                :, :-1, :
            ]  # Shape: (batch_size * k, seq_len, hidden_size)

            # Normalize hidden states
            h_v_norm = h_v / (
                h_v.norm(dim=-1, keepdim=True) + 1e-12
            )  # Shape: (batch_size * k, hidden_size)
            h_j_norm = h_j / (
                h_j.norm(dim=-1, keepdim=True) + 1e-12
            )  # Shape: (batch_size * k, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim = torch.bmm(
                h_v_norm.unsqueeze(1), h_j_norm.transpose(1, 2)
            ).squeeze(
                1
            )  # Shape: (batch_size * k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=-1)[0]  # Shape: (batch_size * k,)

            # Compute scores: align topk_probs to (batch_size * k,)
            P_LM_v = topk_probs.reshape(-1).exp()  # Shape: (batch_size * k,)
            scores = P_LM_v * max_sim  # Shape: (batch_size * k,)

            # For each original batch element select the best among its k candidates
            scores_per_batch = scores.view(batch_size, k)  # Shape: (batch_size, k)
            best_idx_per_batch = scores_per_batch.argmax(dim=1)  # Shape: (batch_size,)

            # Gather the best tokens for each batch element
            batch_indices = torch.arange(batch_size, device=device)
            best_tokens = topk_indices[batch_indices, best_idx_per_batch].unsqueeze(
                1
            )  # Shape: (batch_size, 1)

            # Append the selected tokens to the target sequences
            tgt_input = torch.cat(
                [tgt_input, best_tokens], dim=1
            )  # Shape: (batch_size, seq_len + 1)

            # Update finished condition per batch element and stop if all finished
            if "finished" not in locals():
                finished = torch.zeros(batch_size, dtype=torch.bool, device=device)
            finished = finished | (best_tokens.squeeze(1) == EOS_token)
            if finished.all():
                break
